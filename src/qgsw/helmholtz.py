...
def compute_capacitance_matrices(
    ...
    inv_cap_matrices = torch.zeros(
        (nl, M, M), dtype=torch.float64, device="cpu"
    )
    rhs = torch.zeros(
...
if __name__ == "__main__":
    ...
    helmholtz_dirichletbc = (
        lambda f, dx, dy, lambd: (
    ...
    fig.suptitle(
        "Solving Dirichlet-BC Helmholtz equation on square domain with DSTI"
    )
    fig.tight_layout()
    ...
    fig.colorbar(
        ax[1].imshow(
            diff_ma.T, vmin=-vM, vmax=vM, origin="lower", cmap=palette
        ),
        ax=ax[1],
    )
    fig.suptitle(
        "Solving Dirichlet-BC Helmholtz eq. $\\Delta f - f = r$ on circular domain with CMM and DST-I"
    )
    (
        ax[0].set_xticks([]),
        ax[1].set_xticks([]),
    ...
    # f = torch.zeros(1, nx, ny, dtype=dtype, device=device).normal_()

    # mask = torch.ones(nx, ny, dtype=dtype, device=device)
    # f_mask = torch.zeros(1, nx, ny, dtype=dtype, device=device).normal_()
    # for im, jm in [(nx//4,ny//4), ((3*nx)//4,(3*ny)//4)]:
    # mask[im,jm] = 0
    # v = f_mask[...,im,jm]
    # f_mask[...,im-1,jm] = v
    # f_mask[...,im+1,jm] = v
    ...
    # for f_, mask_, domain in [(f, torch.ones_like(mask), 'square domain'),
    # (f_mask, mask, 'masked domain')]:

    # solver = HelmholtzNeumannSolver(
    # nx, ny, dx, dy, lambd, dtype, device, mask=mask_)
