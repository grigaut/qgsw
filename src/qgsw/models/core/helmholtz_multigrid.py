...
def prolong(v, divisor=16):
    ...
    nx, ny = v.shape[-2:]
    v_f = torch.zeros(
        v.shape[:-2] + (2 * nx, 2 * ny), dtype=v.dtype, device=v.device
    )
    v_ = F.pad(v, (1, 1, 1, 1))
    ...
    v_f[_2ip1_2jp1] = (
        9 * v_[_i_j] + 3 * (v_[_ip1_j] + v_[_i_jp1]) + v_[_ip1_jp1]
    )
...
class MG_Helmholtz:
    ...
    def __init__(
        ...
        self.omega_pre = torch.tensor(
            0.95, dtype=self.dtype, device=self.device
        )
        self.omega_post = torch.tensor(
            0.95, dtype=self.dtype, device=self.device
        )
        ...
        if self.niter_bottom <= 0:
            ...
            self.i_s = i_s
            self.j_s = j_s
            self.bottom_inv_mat = torch.linalg.pinv(
                h_mat.to(device=DEVICE)
            ).to(self.device)
        ...
        if use_compilation:
            ...
            divisor_restrict = torch.ones(
                nx // 2, ny // 2, dtype=self.dtype, device=self.device
            )
            self.restrict = torch.jit.trace(restrict, (f, divisor_restrict))
            divisor_prolong = torch.ones(
                2 * nx, 2 * ny, dtype=self.dtype, device=self.device
            )
    ...
    def solve_V(self, rhs):
        ...
        # print(f'init resnorm: {(res.norm()/f.norm()).to(device=DEVICE).item():.3e}')
        ...
        #       f'{(res.norm()/f.norm()).to(device=DEVICE).item():.3E}')
    ...
    def FMG_Helmholtz(self, rhs, dx, dy):
        ...
        while nite < self.max_ite and res.norm() / f.norm() > self.tol:
            ...
            nite += 1
        # print(f'Number or V-cycle: {nite}, {(res.norm()/f.norm()).to(device=DEVICE).item():.3E}')
