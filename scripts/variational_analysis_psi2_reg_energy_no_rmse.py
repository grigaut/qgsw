...
def energy(
    ...
    dtq1 = (
        ...
        * (1 / H1 / g1 + 1 / H1 / g2)
        * dpsi1[:, :1, 1:-1, 1:-1]
        + beta_plane.f0**2 * 1 / H1 / g2 * dpsi2[:, :1, 1:-1, 1:-1]
    )
    dtq1 = interpolate(dtq1[..., 3:-3, 3:-3])
    q1 = (
        laplacian(psi1, dx, dy)
        - beta_plane.f0**2
        * (1 / H1 / g1 + 1 / H1 / g2)
        * psi1[:, :1, 1:-1, 1:-1]
        + beta_plane.f0**2 * 1 / H1 / g2 * psi2[:, :1, 1:-1, 1:-1]
    )

    u1, v1 = grad_perp(psi1[..., 4:-4, 4:-4])
    u1 /= dx
    v1 /= dy

    dq_1 = div_flux_5pts_only(interpolate(q1), u1, v1, dx, dy)
    ...
    return (
        interpolate(psi1[..., 4:-4, 4:-4]) * H1 * (dtq1 + dq_1)
        + interpolate(psi2[..., 4:-4, 4:-4]) * H2 * (dtq2 + dq_2)
    ).sum()
    ...
    for o in range(optim_max_step):
        ...
        with torch.enable_grad():
            ...
            energy_change = (
                energy(
                    psis[-2],
                    psi2 + (n_steps_per_cyle - 2) * dt * dpsi2,
                    (psis[-1] - psis[-2]) / dt,
                    dpsi2,
                )
                - energy(psis[0], psi2, (psis[1] - psis[0]) / dt, dpsi2)
            ).abs()
            loss += gamma_2 * energy_change
