...
for c in range(n_cycles):
    ...
    psi2 = (torch.rand_like(psi0)).requires_grad_()
    ...
    for o in range(optim_max_step):
        ...
        with torch.enable_grad():
            q0 = compute_q_psi2(psi0, torch.tanh(psi2) * 1e4)[
                ..., (p - 1) : -(p - 1), (p - 1) : -(p - 1)
            ]
            q_bcs = [
                Boundaries.extract(
                    compute_q_psi2(
                        psi[:, :1],
                        torch.tanh(psi2) * 1e4 + n * dt * dpsi2,
                    ),
                    p - 2,
                    -(p - 1),
                    p - 2,
...
f = output_dir.joinpath(
    f"results_psi2_reg_tanh_{imin}_{imax}_{jmin}_{jmax}.pt"
)
