...
def rmse_vort(psi: torch.Tensor, psi_ref: torch.Tensor) -> float:
    """RMSE."""
    v = laplacian(psi, dx, dy)
    v_ref = laplacian(psi_ref, dx, dy)
    return ((v - v_ref) / U * L).square().mean()
