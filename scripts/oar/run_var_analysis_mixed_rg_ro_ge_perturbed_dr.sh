...
SRCDIR=$HOME/qgsw
cd $SRCDIR
chmod +x scripts/bash/run_va_mixed_rg_ro_ge_perturbed_dr.sh
...
cmd="./scripts/bash/run_va_mixed_rg_ro_ge_perturbed_dr.sh"
for arg in "${args[@]}"; do
    cmd+=" $arg"
done
...
if [ "$contiguous" = true ]; then
    ...
    oarsub $OAR_OPTS "$combined_cmd" -n "VA-ge-pert-dr-contiguous"
else
    # Run commands as separate jobs
    oarsub -S "$cmd1" -n "VA-ge-pert-dr-[32 96 64 192]"
    oarsub -S "$cmd2" -n "VA-ge-pert-dr-[32 96 256 384]"
    oarsub -S "$cmd3" -n "VA-ge-pert-dr-[112 176 64 192]"
    oarsub -S "$cmd4" -n "VA-ge-pert-dr-[112 176 256 384]"
fi
