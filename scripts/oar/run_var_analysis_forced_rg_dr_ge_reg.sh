...
if [ "$contiguous" = true ]; then
    ...
    oarsub $OAR_OPTS -S "$combined_cmd" -n "VA-frg-dr-ge-reg-contiguous"
else
    # Run commands as separate jobs
    oarsub -S "$cmd1" -n "VA-frg-dr-ge-reg-[32 96 64 192]"
    oarsub -S "$cmd2" -n "VA-frg-dr-ge-reg-[32 96 256 384]"
    oarsub -S "$cmd3" -n "VA-frg-dr-ge-reg-[112 176 64 192]"
    oarsub -S "$cmd4" -n "VA-frg-dr-ge-reg-[112 176 256 384]"
fi
