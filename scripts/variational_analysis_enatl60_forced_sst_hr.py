...
from qgsw.models.qg.psiq.mixed_layer.forced import QGPSIQSSTForced
...
if __name__ == "__main__":
    ...
    args = ScriptsArgsParser.va_setup(
        prefix_default="results_enatl60_forced_sst_hr",
        cycles_default=4,
    )
    ...
    model = QGPSIQSSTForced(
        space_2d=space_interior,
        H=H[:1] * H[1:2] / (H[:1] + H[1:2]),
        beta_plane=beta_plane,
        g_prime=g_prime[1:2],
    )
    model: QGPSIQSSTForced = set_inhomogeneous_model(model)
