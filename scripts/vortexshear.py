...
for Ro in [
    ...
    w_qg = (qg_ml.omega.squeeze() / qg_ml.area).to(device=DEVICE).numpy()
    ...
    wa_0 = qg_ml.omega_a.squeeze().to(device=DEVICE).numpy()
    ...
    if plots_required:
        mask = sw_ml.masks.not_w[0, 0].to(device=DEVICE).numpy()
    ...
    for n in range(n_steps + 1):
        if plots_required and (n % freq_plot == 0 or n == n_steps):
            w_qg = (
                (qg_ml.omega / qg_ml.area / qg_ml.f0).to(device=DEVICE).numpy()
            )
            w_sw = (
                (sw_ml.omega / sw_ml.area / sw_ml.f0).to(device=DEVICE).numpy()
            )
