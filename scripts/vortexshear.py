...
H = torch.tensor([[[400]]], dtype=dtype, device=device)
## ---------------------------------------------------------------------------
...
#     g_prime[0, 0, 0] = 10.0
# 1-layer reduced gravity
g_prime = torch.tensor([[[0.05]]], dtype=dtype, device=device)
## ---------------------------------------------------------------------------
...
for Bu, Ro in [
    ...
    ## Set initial U, V and H ------------------------------------------------
    qg_multilayer.u = torch.clone(u_init)
    qg_multilayer.v = torch.clone(v_init)
    qg_multilayer.h = torch.clone(h_init)
    ## -----------------------------------------------------------------------

    ## Compute diagnostic variable using the new U, V and H ------------------
    # This is required before performing a model step to ensure that
    # diagnostic variables used for advection are updated
