...
from qgsw.decomposition.fourier import SpaceOnlyFourierBasis
...
args = ScriptArgsVA.from_cli(
    ...
    cycles_default=3,
    prefix_default="results_mixed_decomp",
)
specs = defaults.get()
...
beta_effect: torch.Tensor = beta_plane.beta * (y - y0)
...
for c in range(n_cycles):
    ...
    basis_psi2 = SpaceOnlyFourierBasis(
        space_slice_ww.psi.xy.x - space_slice_ww.psi.xy.x[:1, :],
        space_slice_ww.psi.xy.y - space_slice_ww.psi.xy.y[:, :1],
        order=20,
        Lx_max=((H1 + H2) * g1).sqrt() / beta_plane.f0,
        Ly_max=((H1 + H2) * g1).sqrt() / beta_plane.f0,
    )
    basis_dpsi2 = SpaceOnlyFourierBasis(
        space_slice_ww.psi.xy.x - space_slice_ww.psi.xy.x[:1, :],
        space_slice_ww.psi.xy.y - space_slice_ww.psi.xy.y[:, :1],
        order=20,
        Lx_max=((H1 + H2) * g1).sqrt() / beta_plane.f0,
    ...
    msg = f"ѱ₂ -> using basis of order {basis_psi2.order}"
    logger.info(msg)
    msg = f"dѱ₂ -> using basis of order {basis_dpsi2.order}"
    logger.info(msg)

    coefs_psi2 = basis_psi2.generate_random_coefs()
    coefs_psi2 = (torch.randn_like(coefs_psi2) * 1e-2).requires_grad_()
    coefs_dpsi2 = basis_dpsi2.generate_random_coefs()
    coefs_dpsi2 = (torch.randn_like(coefs_dpsi2) * 1e-4).requires_grad_()

    alpha = torch.tensor(0.5, **specs, requires_grad=True)

    numel = alpha.numel() + coefs_psi2.numel() + coefs_dpsi2.numel()
    ...
    basis_dpsi2.set_coefs(coefs_dpsi2)

    register_params_mixed = RegisterParams(
        alpha=alpha,
        psi2=basis_psi2.compute() * psi0_mean,
        dpsi2=basis_dpsi2.compute(),
    )
