...
from qgsw.models.qg.psiq.modified.forced import QGPSIQForcedMDWVDR
...
args = ScriptArgsVA.from_cli(
    ...
    cycles_default=3,
    prefix_default="results_forced_md_ge_g5_dr",
)
specs = defaults.get()
...
def compute_regularization_func(
    ...
    def compute_reg(
        ...
        Args:
            ...
            kappa (torch.Tensor): Deformation radius coefficient.
...
model = QGPSIQForcedMDWVDR(
    space_2d=space_slice,
    H=H[:2],
    beta_plane=beta_plane,
    g_prime=g_prime[:2],
)
model: QGPSIQForcedMDWVDR = set_inhomogeneous_model(model)
...
for c in range(n_cycles):
    ...
    register_params = RegisterParams(
        alpha=alpha, kappa=kappa, coefs=coefs_scaled.to_dict()
    )
    ...
    for o in range(optim_max_step):
        ...
        with torch.enable_grad():
            ...
            compute_reg = compute_regularization_func(basis, space_slice)

            compute_psi2 = basis.localize(xx, yy)

            q0 = crop(
                compute_q_psi2(psi0, compute_psi2(model.time), kappa),
                p - 1,
            )
            ...
            psis_ = (
                (
                    p[:, :1],
                    compute_psi2(model.time + n * model.dt),
                )
            ...
            for n in range(1, n_steps_per_cyle):
                ...
                reg = gamma * compute_reg(psi1_, dpsi1_, kappa, time)
                loss += reg
    ...
    output = {
        ...
        "kappa": register_params.params["kappa"],
