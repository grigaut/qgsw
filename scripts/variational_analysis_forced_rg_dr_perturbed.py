...
model = QGPSIQForced(
    ...
    H=torch.tensor([H1_], **specs)
    * torch.tensor([H2_], **specs)
    / (torch.tensor([H1_], **specs) + torch.tensor([H2_], **specs)),
    beta_plane=beta_plane,
    g_prime=torch.tensor([g2_], **specs),
)
model: QGPSIQForced = set_inhomogeneous_model(model)
model.wind_scaling = torch.tensor([H1_]).item()
if not args.no_wind:
