...
model = QGPSIQForcedSurfML(
    space_2d=space_interior,
    H=H[:2],
    beta_plane=beta_plane,
    g_prime=g_prime[:2],
)
model: QGPSIQForcedSurfML = set_inhomogeneous_model(model)
...
for c in range(n_cycles):
    ...
    wv = WaveletBasis(space_params, time_params)
    wv.n_theta = 7

    msg = f"Using basis of order {wv.order}"
    logger.info(msg)

    coefs_wv = wv.generate_random_coefs()
    coefs_wv = DecompositionCoefs.zeros_like(coefs_wv)
    coefs_wv = coefs_wv.requires_grad_()
    ...
    if with_alpha:
        ...
        numel = coefs.numel() + coefs_wv.numel()
        params = [
            {
                "params": list(coefs.values()),
                "lr": 1e0,
    ...
    for o in range(optim_max_step):
        ...
        with torch.enable_grad():
            ...
            coefs_wv_scaled = coefs_wv.scale(
                *(U**2 / L**2 for _ in range(wv.order))
            )

            wv.set_coefs(coefs_wv_scaled)
            ...
            for n in range(1, n_steps_per_cyle):
                ...
                model.forcing = fwv(model.time)[None, None, ...]
                model.step()
        ...
        torch.nn.utils.clip_grad_norm_(list(coefs_wv.values()), max_norm=1e0)
    ...
    output = {
        ...
        "coefs_wv": register_params.params["coefs_wv"],
