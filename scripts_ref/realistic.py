...
parser.add_argument(
    "--freq_plot", type=int, default=0, help="plot frequence in days"
)
...
mask_land = skimage.morphology.area_closing(
    mask_land, area_threshold=lake_min_area
)
...
mask_ocean = skimage.morphology.area_closing(
    mask_ocean, area_threshold=island_min_area
)
...
f = f0 + beta * (y - y.mean())
print(
    f"Coriolis param min {f.min().cpu().item():.2e}, {f.max().cpu().item():.2e}"
)
